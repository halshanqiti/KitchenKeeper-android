...
public final class KitchenStore extends SQLiteOpenHelper {
    ...
    public static void validate(String type,JSONObject o){
        ...
        if(!type.equals("recipes")&&(o.optString("unit").trim().isEmpty()||o.optString("unit").length()>30||o.optString("location").trim().isEmpty()||o.optString("location").trim().length()>60))throw new IllegalArgumentException("Choose a unit of up to 30 characters and a storage place of up to 60 characters.");
        if(type.equals("recipes")){double rating=o.optDouble("rating",Double.NaN),minutes=o.optDouble("minutes",Double.NaN);if(!Double.isFinite(rating)||rating<0||rating>5||rating!=Math.rint(rating)||!Double.isFinite(minutes)||minutes<0||minutes>100000||minutes!=Math.rint(minutes))throw new IllegalArgumentException("Use whole numbers for recipe minutes and the 0–5 rating.");}
