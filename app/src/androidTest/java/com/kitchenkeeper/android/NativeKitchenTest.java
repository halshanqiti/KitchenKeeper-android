...
public class NativeKitchenTest {
    ...
    @Test public void storageRecipesAndBatchesRemainCorrect()throws Exception{
        ...
        JSONObject corrupt=copy(backup);JSONObject invalid=blankItem("grocery");put(invalid,"name","Bad date");put(invalid,"expiry","2026-99-99");corrupt.getJSONArray("items").put(invalid);try{store.importData(corrupt,new HashSet<>());fail("Invalid import must fail");}catch(IllegalArgumentException expected){}assertEquals(2,store.records("items").size());
    }
    ...
    @Test public void nativeUiSavesRecordsAndSurvivesRecreation()throws Exception{
        try(ActivityScenario<MainActivity> scenario=ActivityScenario.launch(MainActivity.class)){
          ...
          onView(withText("Food")).perform(click());onView(withText("Add grocery")).perform(click());onView(withTagValue(is((Object)"name"))).perform(scrollTo(),replaceText("Native test milk"),closeSoftKeyboard());scenario.recreate();onView(withTagValue(is((Object)"name"))).check(matches(withText("Native test milk")));onView(withText("Save item")).perform(scrollTo(),click());onView(withText("Native test milk")).check(matches(isDisplayed()));
          onView(withText("Recipes")).perform(click());onView(withText("Save a recipe")).perform(click());onView(withTagValue(is((Object)"title"))).perform(scrollTo(),replaceText("My first native recipe"),closeSoftKeyboard());onView(withTagValue(is((Object)"method"))).perform(scrollTo(),replaceText("My own method, saved offline."),closeSoftKeyboard());onView(withText("Save recipe")).perform(scrollTo(),click());onView(withText("My first native recipe")).check(matches(isDisplayed()));capture("native-recipes.png");
          onView(withText("Kitchen")).perform(click());capture("native-kitchen.png");
          onView(withText("Food")).perform(click());onView(withText("Add grocery")).perform(click());onView(withTagValue(is((Object)"name"))).perform(scrollTo(),replaceText("A reviewed product draft"),closeSoftKeyboard());capture("native-editor.png");
        }
